...
from detectron2.modeling import build_anchor_generator, PROPOSAL_GENERATOR_REGISTRY, build_rpn_head
...
class ThunderNetRPNHead(nn.Module):
    """
    RPN classification and regression heads. Uses a 5x5 depthwise conv and a 1x1 conv
    to produce a shared hidden state from which one 1x1 conv predicts objectness logits
    for each anchor and a second 1x1 conv predicts bounding-box deltas specifying how to
...
class ThunderNetRPN(nn.Module):
    ...
    def rpn_features_shape(self):
        shape = {}
        for f in self.in_features:
            shape["{}_rpn_features".format(f)] = ShapeSpec(channels=self.rpn_head.rpn_feature_out_channels,
                                                           height=self.input_shape[f].height,
                                                           width=self.input_shape[f].width,
                                                           stride=self.input_shape[f].stride)
        return shape
